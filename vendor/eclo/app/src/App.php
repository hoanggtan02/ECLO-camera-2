...
class App{
    ...
    // Gọi phương thức động của Medoo và Upload
    public function __call($method, $args) {
        // Nếu Medoo có phương thức đó, gọi nó
        if (self::$database instanceof Medoo && method_exists(self::$database, $method)) {
            return call_user_func_array([self::$database, $method], $args);
        }
        // Nếu Upload có phương thức đó, gọi nó
        if (class_exists('Verot\Upload\Upload')) {
            $upload = new Upload($_FILES['image_field'] ?? null);
            if (method_exists($upload, $method)) {
                return call_user_func_array([$upload, $method], $args);
            }
        }
    ...
    public function router($route, $methods, $callback = null) {
        ...
        if (str_ends_with($route, '::error')) {
            $parts = explode('::error', $route);
            $mainRoute = $parts[0];
            $this->groupErrorRoute[$mainRoute] = $route;
        }
        foreach ($methods as $method) {
            $this->registerRoute(strtoupper($method), $route, $callback);
        }
        return $this;
    }
    ...
    private function registerRoute($methods, $route, $callback = null){
        ...
        if ($route && $callback) {
            ...
            foreach ($methods as $method) {
                $this->routes[strtoupper($method)][$route] = function($vars) use ($callback) {
                    $callback($vars);
                };
            }
        ...
        foreach ($this->routes[$method] as $route => $callback) {
            ...
            if (preg_match($pattern, $path, $matches)) {
                return [
                    'route' => $route,
                    'vars' => array_merge(
                        ['_method' => $method], // Thêm phương thức vào mảng
    ...
    private function handleWildcardRoute($path) {
        $matchedPrefix = '';
        foreach (array_keys($this->groupErrorRoute) as $prefix) {
        ...
        if ($matchedPrefix !== '') {
            $this->routes['GET'][$matchedPrefix.'::error'](['path' => $path]);
            return;
        }
        if (isset($this->routes['GET']['::error'])) {
            $this->routes['GET']['::error'](['path' => $path]);
        } 
        else {
            echo "404 Not Found";
        }
    ...
    public function deleteCookie($name) {
        $path = isset($this->cookies[$name]['path']) ? $this->cookies[$name]['path'] : '/';
        $domain = isset($this->cookies[$name]['domain']) ? $this->cookies[$name]['domain'] : '';
        $secure = isset($this->cookies[$name]['secure']) ? $this->cookies[$name]['secure'] : false;
        $httponly = isset($this->cookies[$name]['httponly']) ? $this->cookies[$name]['httponly'] : false;
        unset($_COOKIE[$name]);
    ...
    // Ngăn người dùng spam API bằng cách giới hạn số request trong khoảng thời gian ngắn.
    public function rateLimit($key, $limit = 10, $time = 60) {
        $ip = $_SERVER['REMOTE_ADDR'];
        $cacheKey = "rate_limit:$key:$ip";
        // Giả lập cache bằng session (có thể dùng Redis hoặc Memcached)
